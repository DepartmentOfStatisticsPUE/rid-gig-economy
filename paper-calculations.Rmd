...
selectivv[[3]] %>%
  ...
  mutate(geo = factor(geo, c("cities", "fua", "woj"),
                      c("Cities", "Functional UAs", "Voivodeships"))) %>%
  group_by(apps, geo) %>%
  ...
  labs(x = "Period (reference=2018.1)", y = "Change in the number of users\n(in thousands)", color = "Apps") +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1),
        legend.position = "bottom") ->  p1b
...
selectivv[[3]] %>%
  ...
  mutate(region_name = str_replace(region_name, "Functional area - ", "FUA: "),
         group = factor(group, c("Cities", "Functional area", "Voivodeship"),
                        ordered = T))  %>%
  ...
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1),
        legend.position = "bottom") -> p1_annex
